...
#ifdef __CINT__
void make_training_and_test_datasets(int stride = 100000, int offset = 0)
{
...
const Cut kSel = kNumuQuality && kNumuContainFD2017 && kIsNumuCC;
...
void make_training_and_test_datasets(int stride = 100000, int offset = 0)
{
  string dataset = "defname: prod_caf_R17-11-14-prod4reco.d_fd_genie_nonswap_fhc_nova_v08_full_v1 with stride "+to_string(stride)+" offset "+to_string(offset);
  /// print with number of tracks info
  MakeTextListFile(dataset, {kSel && kMuonIdentified}, {"fd_standard_numucc_selection_stride"+to_string(stride)+"_offset"+to_string(offset)+".txt"},
                   {&kRun, &kSubRun, &kCycle, &kEvt, &kSlice, &kTrueE, &kTrueMuonE, &kTrkLength, &kMuStopZ, &kTrueHadE, &kNumuHadVisE, &kRecoQ2,
                   &kCVNFSChargedPionScore, &kNPng},
                   &kStandardSpillCuts);
}
#endif
