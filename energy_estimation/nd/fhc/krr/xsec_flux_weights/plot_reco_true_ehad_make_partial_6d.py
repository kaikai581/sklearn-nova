...
def make_plot(apar, gpar, scaledown, offset, modelstep, modeloffset):
  ...
  offwei = root2array('../training_data.root', branches='weight',
                      selection=cut,
                      step=scaledown, start=offset)
  X6d = root2array('../training_data.root',
                   branches=['calehad', 'cvnpi0', 'cvnchargedpion', 'cvnneutron', 'cvnproton', 'npng'],
                   selection=cut,
                   step=scaledown, start=offset)
  X6d = X6d.view(np.float32).reshape(X6d.shape + (-1,))
  ...
  y_pred = None
  pscaler = 'models/6d/sample_weighted_hadronic_6d_scaler_a{}g{}step{}offset{}.pkl'.format(apar, gpar, modelstep, modeloffset)
  pmodel = 'models/6d/sample_weighted_hadronic_6d_energy_estimator_a{}g{}step{}offset{}.pkl'.format(apar, gpar, modelstep, modeloffset)
  if os.path.isfile(pscaler):
    if os.path.isfile(pmodel):
      y_pred = make_prediction(pscaler, pmodel, X6d)
      plt.hist(y_pred, bins=bins, histtype='step', color='green', weights=offwei)
  
  # ~ plt.xlim(0,2)
  # ~ plt.show()
  # save the three histograms
  os.system('mkdir -p partial_output/6d')
  joblib.dump(y, 'partial_output/6d/sample_weighted_hadronic_true_a{}g{}step{}offset{}.pkl'.format(apar, gpar, scaledown, offset))
  joblib.dump(yoff, 'partial_output/6d/sample_weighted_hadronic_official_a{}g{}step{}offset{}.pkl'.format(apar, gpar, scaledown, offset))
  joblib.dump(y_pred, 'partial_output/6d/sample_weighted_hadronic_krr_a{}g{}step{}offset{}.pkl'.format(apar, gpar, scaledown, offset))
  joblib.dump(offwei, 'partial_output/6d/sample_weighted_hadronic_weight_a{}g{}step{}offset{}.pkl'.format(apar, gpar, scaledown, offset))
