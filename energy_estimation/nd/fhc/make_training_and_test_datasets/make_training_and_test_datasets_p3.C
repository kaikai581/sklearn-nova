...
#ifdef __CINT__
void make_training_and_test_datasets_p3(int stride = 100000, int offset = 0)
{
...
void make_training_and_test_datasets_p3(int stride = 100000, int offset = 0)
{
  string dataset = "defname: prod_caf_R17-11-14-prod4reco.d_nd_genie_nonswap_fhc_nova_v08_period3_v1 with stride "+to_string(stride)+" offset "+to_string(offset);
  /// print with number of tracks info
  MakeTextListFile(dataset, {kNumuCutND2018}, {"nd_p3_standard_numucc_selection_stride"+to_string(stride)+"_offset"+to_string(offset)+".txt"},
