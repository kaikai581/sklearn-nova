...
void make_training_and_test_datasets(int stride = 100000, int offset = 0)
{
  ...
  MakeTextListFile(dataset, {kSel && kMuonIdentified}, {"standard_numucc_selection_stride"+to_string(stride)+"_offset"+to_string(offset)+".txt"},
//		   {&kRun, &kSubRun, &kCycle, &kEvt, &kSlice, &kTrueE, &kTrueMuonE, &kTrkLenAct, &kTrkLenCat, &kMuStopZ, &kTrueHadE, &kHadAll, &kRecoQ2},
		   {&kRun, &kSubRun, &kCycle, &kEvt, &kSlice, &kTrueE, &kTrueMuonE, &kTrkLenAct, &kTrkLenCat, &kMuStopZ, &kTrueHadE, &kHadAll, &kRecoQ2,
